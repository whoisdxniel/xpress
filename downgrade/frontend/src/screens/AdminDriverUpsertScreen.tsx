...
function inferImageMimeType(nameOrUri: string): string {
  const value = nameOrUri.toLowerCase();
  if (/\.png(?:$|\?)/.test(value)) return "image/png";
  if (/\.gif(?:$|\?)/.test(value)) return "image/gif";
  if (/\.webp(?:$|\?)/.test(value)) return "image/webp";
  if (/\.heic(?:$|\?)/.test(value)) return "image/heic";
  if (/\.heif(?:$|\?)/.test(value)) return "image/heif";
  return "image/jpeg";
}
...
export function AdminDriverUpsertScreen({ route, navigation }: Props) {
  ...
  async function pickAndUploadImage(category: string, opts?: { allowsEditing?: boolean }) {
    ...
    const mimeType = inferImageMimeType(asset.fileName ?? asset.uri);
